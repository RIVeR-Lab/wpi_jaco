...
JacoArm::JacoArm()
{
  ...
  for (int joint_id = 0; joint_id < NUM_JACO_JOINTS; ++joint_id)
  {
    ...
    hardware_interface::JointStateHandle state_handle(joint_name, &joint_pos[joint_id], &joint_vel[joint_id],
                                                      &joint_eff[joint_id]);
  ...
  for (int finger_id = 0; finger_id < NUM_JACO_FINGER_JOINTS; ++finger_id)
  {
    ...
    hardware_interface::JointStateHandle state_handle(finger_name, &finger_pos[finger_id], &finger_vel[finger_id],
                                                      &finger_eff[finger_id]);
    jnt_state_interface.registerHandle(state_handle);
